...
public class MoffEntity extends TamableAnimal {
	public MoffEntity(EntityType<MoffEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(7, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = MoffEntity.this.getRandom();
				double dir_x = MoffEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = MoffEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = MoffEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
		...
		this.goalSelector.addGoal(9, new Goal() {
			...
			public boolean canUse() {
				if (MoffEntity.this.getTarget() != null && !MoffEntity.this.getMoveControl().hasWanted()) {
					return true;
				} else {
					return false;
				}
			...
			public boolean canContinueToUse() {
				return MoffEntity.this.getMoveControl().hasWanted() && MoffEntity.this.getTarget() != null && MoffEntity.this.getTarget().isAlive();
			}

			@Override
			public void start() {
				LivingEntity livingentity = MoffEntity.this.getTarget();
				Vec3 vec3d = livingentity.getEyePosition(1);
				MoffEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 2);
			}

			@Override
			public void tick() {
				LivingEntity livingentity = MoffEntity.this.getTarget();
				if (MoffEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
					MoffEntity.this.doHurtTarget(livingentity);
				} else {
					double d0 = MoffEntity.this.distanceToSqr(livingentity);
					if (d0 < 16) {
						Vec3 vec3d = livingentity.getEyePosition(1);
						MoffEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 2);
					}
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		MoffEntity retval = PemtsModEntities.MOFF.create(serverWorld);
	...
	public static void init() {
		BiomeModifications.create(new ResourceLocation(PemtsMod.MODID, "moff_entity_spawn")).add(ModificationPhase.ADDITIONS,
				...
				ctx -> ctx.getSpawnSettings().addSpawn(MobCategory.AMBIENT, new MobSpawnSettings.SpawnerData(PemtsModEntities.MOFF, 20, 1, 4)));
	}
