...
public class MaowEntity extends TamableAnimal {
	public MaowEntity(EntityType<MaowEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	public void playerTouch(Player sourceentity) {
		...
		double y = this.getY();
		double z = this.getZ();
		MaowEntityDiesProcedure.execute(world, x, y, z, entity);
	}

	@Override
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		MaowEntity retval = PemtsModEntities.MAOW.create(serverWorld);
	...
	public boolean isFood(ItemStack stack) {
		return Ingredient.of(new ItemStack(PemtsModItems.FLESH), new ItemStack(PemtsModItems.HAND)).test(stack);
	}

	public static void init() {
		BiomeModifications.create(new ResourceLocation(PemtsMod.MODID, "maow_entity_spawn")).add(ModificationPhase.ADDITIONS,
				BiomeSelectors.includeByKey(ResourceKey.create(Registries.BIOME, new ResourceLocation("cherry_grove")), ResourceKey.create(Registries.BIOME, new ResourceLocation("grove")),
						ResourceKey.create(Registries.BIOME, new ResourceLocation("flower_forest")), ResourceKey.create(Registries.BIOME, new ResourceLocation("meadow"))),
				ctx -> ctx.getSpawnSettings().addSpawn(MobCategory.AMBIENT, new MobSpawnSettings.SpawnerData(PemtsModEntities.MAOW, 20, 1, 4)));
	}
